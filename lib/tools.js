...
const gmail = {
  ...
  // Saves a draft — never sends. threadId keeps it attached to the original
  // conversation; in-reply-to/references are best-effort (only set when the
  // original message's Message-ID header was available) so most mail clients
  // still thread it correctly even without them.
  async create_draft(token, input){
    ...
    const j = await call(`${G}/drafts`, token,
      { method: "POST", body: JSON.stringify({ message: { raw, threadId: input.threadId } }) });
    return { id: j.id, messageId: j.message?.id };
  },
};
...
const calendar = {
  ...
  async update_event(token, input){
    // Partial update — Google Calendar's PATCH only touches the fields sent,
    // so moving a meeting doesn't require re-sending its attendees/description.
    const body = {};
    if (input.summary !== undefined) body.summary = input.summary;
    if (input.description !== undefined) body.description = input.description;
    if (input.startTime) body.start = { dateTime: input.startTime };
    if (input.endTime) body.end = { dateTime: input.endTime };
    const j = await call(`${C}/${encodeURIComponent(input.eventId)}?sendUpdates=none`, token,
...
/* ---------------- Todoist ----------------
   Todoist retired REST v2 and Sync v9 in favour of one unified surface
   under /api/v1/ — both old bases now return the same 410 notice
   regardless of which one you hit. List endpoints under the new API
   come back wrapped in a pagination envelope ({results: [...],
   next_cursor}) rather than a bare array, so every list read below
   unwraps defensively instead of assuming either shape; single-resource
   endpoints (create/close/update/delete) are unaffected by that change.

   REST priority is inverted against the connector's p1..p4 labels:
   API 4 is urgent (p1) and API 1 is lowest (p4). */
const T = "https://api.todoist.com/api/v1";
const listOf = j => Array.isArray(j) ? j : (j?.results ?? j?.items ?? []);
const toP = n => `p${5 - (n || 1)}`;
...
const todoist = {
  async ["find-tasks-by-date"](token, input){
    ...
    const filter = mode === "overdue-only" ? "overdue"
                 ...
                 : `overdue | ${start}`;
    const j = await call(`${T}/tasks/filter?query=${encodeURIComponent(filter)}`, token);
    const tasks = listOf(j).map(shapeTask).slice(0, input?.limit || 100);
  ...
  async ["find-completed-tasks"](token, input){
    ...
    const j = await call(`${T}/tasks/completed/by_completion_date?${q}`, token);
    return {
      tasks: listOf(j).map(i => ({
        id: i.task_id ?? i.id, content: i.content, completedAt: i.completed_at, checked: true,
      })),
    };
  },
